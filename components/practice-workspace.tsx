 ...
 *
 * `idle`      — learner is still editing, nothing submitted yet.
 * `submitted` — request accepted, queued for evaluation.
 * `evaluating`— evaluation in progress.
 * `completed` — evaluation finished (the real evaluator is not wired up yet,
 *               so no score is shown — see the feedback panel).
 * `failed`    — evaluation could not complete; the learner can retry.
 */
...
export function PracticeWorkspace({ problem }: { problem: Problem }) {
  ...
  /**
   * Simulated evaluation pipeline. There is no evaluator connected yet, so this
   * only advances the visible lifecycle — it never fabricates a score. When the
   * real evaluator lands, replace this with a server action that scores `code`
   * and `notes` and resolves to `completed` (with a result) or `failed`.
   */
  ...
  function handleSubmit() {
    const codeIsEmpty =
      code.trim().length === 0 || code.trim() === problem.starterCode.trim()
    const notesIsEmpty = notes.trim().length === 0

    if (codeIsEmpty && notesIsEmpty) {
      setError(
        'Add your Java solution and design reasoning before submitting.',
      )
      return
    }
    if (codeIsEmpty) {
    ...
    if (notesIsEmpty) {
      setError(
        'Add your design reasoning — explain your key decisions and trade-offs.',
      )
      return
    }
  ...
  return (
    <div className="flex min-w-0 flex-col rounded-xl border border-border bg-card">
      <div className="flex items-center justify-between gap-2 border-b border-border px-3 py-2">
        <div className="flex gap-1">
          ...
          <TabButton
            ...
            onClick={() => hasSubmitted && setTab('feedback')}
          >
            Feedback
          </TabButton>
        </div>
      ...
      {tab === 'solution' ? (
        <div className="flex flex-col gap-4 p-4">
          ...
          <div>
            <label htmlFor="notes" className="mb-1.5 block text-sm font-medium">
              ...
              <span className="ml-2 font-normal text-muted-foreground">
                Explain your responsibilities, abstractions, trade-offs, and
          ...
          <div className="flex items-center justify-between gap-3">
            <p className="text-xs text-muted-foreground">
              Your solution and reasoning are evaluated together.
            </p>
        ...
        <div className="p-4">
          <FeedbackPanel phase={phase} onRetry={handleRetry} />
        </div>
      )}
    </div>
  )
...
function FeedbackPanel({
    ...
    return (
      <div className="flex flex-col items-center gap-2 py-16 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          Submit a solution to start an evaluation.
        </p>
      </div>
    )
  }
      ...
      <div className="flex flex-col items-center gap-3 py-16 text-center">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Something went wrong while evaluating your solution.
          </p>
        </div>
  ...
  return (
    <div className="flex flex-col gap-6 py-4">
      ...
      {phase === 'completed' ? (
        <div className="rounded-lg border border-border bg-secondary/40 p-4 text-sm leading-relaxed text-muted-foreground text-pretty">
          Your solution was submitted and moved through the evaluation pipeline.
          Automated design scoring across the seven criteria will appear here
          once the evaluator is connected — no scores are shown yet because none
          have been generated for your work.
        </div>
      ) : (
        <p className="text-sm text-muted-foreground">
          Hang tight — your solution is moving through the pipeline.
        </p>
      )}
    </div>
  )
